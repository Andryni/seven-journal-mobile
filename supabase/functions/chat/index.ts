...
- These flag names are CONTRACT, not vocabulary. Never write "excursionsFillable", "rFillable", "costsFillable", "isAutoAccount" or any other JSON field name in your reply, never put field names in backticks, and never claim a flag "is false" — the trader has no screen showing flags, so quoting them reads as a malfunction. When "excursionsFillable" is false for the trades in question, say instead that the terminal captured no stop/target levels to resend, and fall back to proposing "request_broker_fill" for the bridge-sourced trades anyway (the terminal rechecks at its next sync) or explaining the fields stay manual. When it is true, say the terminal CAN resend the missing levels and propose the action. - "appliedActions" lists changes the trader ALREADY confirmed in this conversation (add tag, mental state, derived R, broker costs, or a request sent to the terminal), newest last. They happened on the device, after your previous reply: treat them as done. Never describe one as still to do, never re-propose it, and when the trader asks "is it done?", answer from this list. A line saying the terminal was ASKED for missing levels means the request was delivered and the values arrive at the terminal's next sync — the fields are still empty until then, which are two different facts: say the request is in, not that the journal is complete.
...
function sanitizeContext(input: unknown): Record<string, unknown> | null {
  ...
  /**
   * Actions the trader already confirmed on the device, newest last.
   *
   * Optional and purely additive, so a client that never sends it keeps
   * working. Strings are capped hard: they are app-authored sentences, and the
   * only thing this block may ever carry is a description of a change this
   * client already made — never anything the model could have written.
   */
  if (Array.isArray(c.appliedActions)) {
    const lines = c.appliedActions
      .filter((x): x is string => typeof x === 'string')
      .map(x => x.slice(0, 240))
      .slice(-5);
    if (lines.length > 0) contextOut.appliedActions = lines;
  }
